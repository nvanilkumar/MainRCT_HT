...
//echo $search_str;
$qselect="select id,psu,door_no,enc_id,patient_id,vill_name,locality,first_name,sur_name,gender,age,enc_type,enc_date,asha_assigned from cvd_basetable where enc_date is not null and patient_id is not null";
		...
		<tr>
			<td>
				<table border="0" cellspacing=0 cellpadding=0 class="tgrid" align="center" style="border:0px solid #000">
					<tr>
                                        <th style="cursor:pointer;">PSU</th>
						...
						$html.='</th>
						<th style="cursor:pointer;">
							<a onclick="changeTab('.'\'List\''.',\'locality_name\',\''.$negorder.'\',\''.$page.'\',\'locality_name\',\'village_name\',\'hh_head_fname\');" title="Sort By Locality '.$negorder.'" style="text-decoration:none; color:#FFF;">Street</a>';
							
							if($order_by=='person_attr.Locality')
							{
								$html.='<img src="'.$filterimg.'" style="padding-left:3px;"/>';
							}
						$html.='</th> <th> Door No</th>
						<th style="cursor:pointer;">
	...
	if($tabs_res && ($num=db_num_rows($tabs_res))): 
		...
		while ($row = db_fetch_array($tabs_res)) {
			...
			$html.='<tr class="'.$cssclass.'" id="'.$row['id'].'" onclick="onRowSelection(this.id);">                
						<td align="center">&nbsp;'.($row['psu']!=""? strtoupper($row['psu']):"-").'</td>
						<td align="center">&nbsp;'.($row['vill_name']!=""? strtoupper($row['vill_name']):"-").'</td>
						<td align="center">&nbsp;'.($row['locality']!=""? strtoupper($row['locality']):"-").'</td>
						<td align="center">&nbsp;'.($row['door_no']!=""? strtoupper($row['door_no']):"-").'</td>
						<td align="center">&nbsp;'.$row['patient_id'].'</td>';
                                        if(strcmp($_SESSION['_staff']['userID'],"drdp")!=0) {
						$html.='<td align="center">&nbsp;'.strtoupper($row['first_name']).'</td>';
                                        }
