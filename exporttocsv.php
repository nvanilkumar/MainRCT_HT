...
//latest changes made by anil
    $columns = "select column_name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME='cvd_basetable'";
    $cresult = db_query($columns);
     $columns=array();
    if ($cresult && db_num_rows($cresult)) {

        while ($row = db_fetch_row($cresult)) {
//            var_dump($row[0]);
...
//    print_r($columns);
    ...
    $qselect = 'select * from cvd_basetable';
...
function export2AllCSVOnBasetable() {
    ...
    //latest changes made by anil
    $qselect="select * from cvd_basetable where patient_id is not null and enc_date is not null";
    $columns = "select column_name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME='cvd_basetable'";
    $cresult = db_query($columns);
     $columns=array();
    if ($cresult && db_num_rows($cresult)) {
